...
const AddPhysicalGame = () => {
  const [modalIsOpen, setOpenIsModal] = useState<boolean>(false)
  const [gameDetails, setGameDetails] = useState<GameDetailsPhysical>({
    start_at: '',
    place: 0,
    rewards: [],
    geolocation_x: 0,
  ...
  const [geoX, setGeoX] = useState<string>('')
  const [geoY, setGeoY] = useState<string>('')
  ...
  const handleInputGeoX = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/[^0-9.,-]/g, '');
    setGeoX(value);
    const parsedValue = parseFloat(value);
    setGameDetails({...gameDetails, geolocation_x: parsedValue});
  };
  
  const handleInputGeoY = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/[^0-9.,-]/g, '');
    setGeoY(value);
    const parsedValue = parseFloat(value);
    setGameDetails({...gameDetails, geolocation_y: parsedValue});
  };
  ...
  const validateInputs = (): boolean => {
    if (!date || !time || !gameDetails.place || gameDetails.rewards.length === 0 || !geoX || !geoY) {
  ...
  const theme = createTheme({
	...
	return (
		<>
			<div className={`flex justify-between mt-2 items-center text-sm px-2 border-opacity-0 text-white bg-myColors-350 rounded-xl h-[55px]`}  onClick={() => setOpenIsModal(true)}>
			<div className='flex gap-5 items-center ml-2'>
						<div className='flex-col'>
							<p className='text-[12px]'></p>
							<div className='flex items-center'>
								<p className='text-[14px] font-medium'>{t("adminPhysical")}</p>
							</div>
						</div>
					</div>
					<div>
        ...
        <div className="fixed inset-0 bg-myColors-400 flex justify-center items-center z-50">
          <div className="bg-myColors-400 rounded-lg w-full max-w-lg h-[100vh] overflow-y-auto">
            ...
            <div className="p-5">
              <p className='text-xl font-medium mb-4 text-center'>{t("adminCreateGamePhysical")}</p>
              ...
              <div className="mb-5">
                <p className='text-white'>{t("adminGeoLocation")}</p>
                <div className='flex justify-between items-center'>
                  <div className='flex flex-col  w-[45%]'>
                    <p className='text-[11px]'>{t("adminLatidute")}</p>
                  <input
                    type="number"
                    value={geoX}
                    onChange={(e) => handleInputGeoX(e) }
                    className='text-black w-full mt-1 rounded-lg h-[40px] px-2 outline-none'
                  />
                  </div>
                  <div className='flex flex-col w-[45%]'>
                    <p className='text-[11px]'>{t("adminLongitude")}</p>
                  <input
                    type="number"
                    value={geoY}
                    onChange={(e) => handleInputGeoY(e)}
                    className='text-black w-full mt-1 rounded-lg h-[40px] px-2 outline-none'
                  />
                  </div>
...
export default AddPhysicalGame